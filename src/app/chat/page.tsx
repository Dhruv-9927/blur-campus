        ...
        <div className="min-h-screen bg-[#fdfbf7] text-[#2d2420] font-sans p-4">
            ...
            <div className="space-y-4 max-w-md mx-auto">
                {matches && matches.length > 0 ? (
                    (() => {
                        // Deduplicate matches by partner ID
                        ...
                        matches.forEach(match => {
                            ...
                            const existingMatch = uniqueMatchesMap.get(partner.id);

                            // If no match exists for this partner, or the current one has more messages/is newer, use it
                            if (!existingMatch ||
                                match.message_count > existingMatch.message_count ||
                                (match.message_count === existingMatch.message_count && new Date(match.created_at) > new Date(existingMatch.created_at))) {
                                uniqueMatchesMap.set(partner.id, { ...match, partner });
                            }
                        });
